...
from stage import Stage, StageItem, stage_item_draw, TILE_SIZE, isColliding
...
class App:
    ...
    def __init__(self):
        ...
        self.max_spawn_per_stage = [4, 6]  # Max spawn for each stage
    ...
    def reset(self, reset_health=False):
        ...
        self.stage = Stage(pyxel.tilemap(self.stage_level))
        self.spawn_count = 0
        self.enemies = []
        ...
        self.player = Player(self.stage, self.enemies, self.health)
    ...
    def update(self):
        ...
        if self.isGameWon:
            if pyxel.btn(pyxel.KEY_N): #click to proceed to next stage
                ...
                if self.stage_level < len(self.max_spawn_per_stage):
                    self.reset()
                    self.update_heart()
                else:
                    pyxel.quit()  # End the game if there are no more stages
        ...
        for enemy in self.enemies:
            for bullet in enemy.bullets:
                if (
                    ...
                    self.player.health -= 1
                    if self.player.health == 1:
                        self.stage.set(8, 0, StageItem.HEART_OFF)
                        self.playerhealth_IS_ONE = True
                        
                    if self.player.health <= 0:
                        self.stage.set(7, 0, StageItem.HEART_OFF)
    ...
    def draw_cheat(self):
        self.stage.set(8, 0, StageItem.HEART_ON)
    ...
    def draw_player_hit_restart(self):
        pyxel.text(128 / 2 - 25, 128 / 2 - 40, 'Player got hit!', 11)
